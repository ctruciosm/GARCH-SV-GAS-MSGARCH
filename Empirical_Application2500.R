...
for (i in 1:oos) {
  ...
  garch_n_fore <- future_apply(returns_c, 2, function(x) {
      ...
      es <- sapply(var, function(v) mean(fit@fit$residuals[fit@fit$residuals < v]))
  ...
  gas_n_fore <- future_apply(returns_c, 2, function(x) {
    ...
    var <- quantile(e, prob =  alpha) * sigma_fore
    es <- sapply(var, function(v) mean(fit@Data$vY[fit@Data$vY < v]))
  ...
  ms_n_fore <- future_apply(returns_c, 2, function(x) {
    ...
    var <- quantile(e, prob = alpha) * sigma_fore
    es <- sapply(var, function(v) mean(fit$data[fit$data < v]))
  ...
  svb_n_fore <- future_apply(returns_c, 2, function(x) {
    ...
    var <- quantile(e, prob =  alpha) * sigma_fore
    es <- sapply(var, function(v) mean(fit$y[fit$y < v]))
